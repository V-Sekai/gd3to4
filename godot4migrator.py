...
replacements = [
    ...
    (re.compile(r"\byield\s*\(\s*(\S[\s\S]*?)(\s*,\s*)*[\"'](.*)[\"']\s*\)\s*$"), "await \\1.\\3"),
    (re.compile(r"\byield\s*\((.*?)\)\s*$|\byield (.*)$"), "await \\1\\2"),
    (re.compile(r"\b(connect|disconnect|is_connected)(\s*)\((\s*)([^,]*),(\s*)(?!Callable)([^,\s][^,]*),([^,]*)([,)])"), r"\1\2(\3\4,\5Callable(\6,\7)\8"),
    ...
    (re.compile(r'\b@?master(\s*)func\b'), '@rpc(master)\\1func'),
    (re.compile(r'\b@?remote(\s*)func\b'), '@rpc(any)\\1func'),
    (re.compile(r'\b@?remotesync(\s*)func\b'), '@rpc(any,sync)\\1func'),
    (re.compile(r'\b@?puppet(\s*)func\b'), '@rpc(puppet)\\1func'),
    ...
    ('XRPositionalTracker.TRACKER_LEFT_HAND', 'XRPositionalTracker.TrackerHand.TRACKER_HAND_LEFT'),
    ('XRPositionalTracker.TRACKER_RIGHT_HAND', 'XRPositionalTracker.TrackerHand.TRACKER_HAND_RIGHT'),
    ...
    ('to_ascii', 'to_ascii_buffer'),
    ('to_utf8', 'to_utf8_buffer'),
    ('http_escape', 'uri_encode'),
    ('percent_encode', 'uri_encode'),
    ('http_unescape', 'uri_decode'),
    ('percent_decode', 'uri_decode'),
    ('popup_exclusive', 'exclusive'),
    ('clamped', 'limit_length'),
    ('about_to_show', 'about_to_popup'),
    ('property_list_changed_notify', 'notify_property_list_changed'),
    ('enabled_focus_mode', 'focus_mode'),
    ]
